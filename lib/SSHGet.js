...
export class SSHGet extends EventEmitter {
    ...
    getLocalPath(file) {
        if (this.files.length === 1 && !this.isDirectory) {
            // Single file: use destination as filename (or destination/filename if destination ends with /)
    ...
    async downloadFiles() {
        ...
        while (pendingJobs.length > 0 || runningPromises.size > 0) {
            if (this.aborted) {
                throw new Error("Download aborted")
            }

            // Start new jobs for each available tunnel
            while (pendingJobs.length > 0) {
                const tunnel = this.tunnelPool.acquire()
                if (!tunnel) break // No tunnels available

                const job = pendingJobs.shift()
                const promise = runJob(job, tunnel).finally(() => {
