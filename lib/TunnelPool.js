...
export class TunnelPool extends EventEmitter {
    ...
    buildPrimarySSHArgs(localPort) {
        ...
        const remoteCmd = `cd ${shellEscape(this.remotePath)} && python3 -c ${shellEscape(PYTHON_HTTP_SERVER)} ${localPort}`
        args.push(remoteCmd)
    ...
    async startPrimaryTunnel() {
        return new Promise((resolve, reject) => {
            ...
            const sshArgs = this.buildPrimarySSHArgs(localPort)
    ...
    async startSecondaryTunnel(index) {
        ...
        const remotePort = this.basePort // Connect to same remote port as primary
