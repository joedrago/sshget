...
def read_exact(n):
    ...
    while len(d) < n:
        ...
        if not c:
            log(f"stdin closed, exiting (had {len(d)}/{n} bytes)")
            sys.exit(0)
        d += c
    return d
...
def handle_request():
    ...
    try:
        ...
        send_error(f"Invalid path encoding: {e}")
        return

    # Read offset and length
    off, ln = struct.unpack('>QQ', read_exact(16))
    log(f"request: {path} offset={off} len={ln}")
    ...
    try:
        ...
        with open(path, 'rb') as f:
            ...
            while remaining > 0:
                ...
                if not data:
                    log(f"unexpected EOF at {bytes_sent}/{actual_len}")
                    break
                stdout.write(data)
                bytes_sent += len(data)
                remaining -= len(data)
